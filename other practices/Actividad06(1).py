...
class Grafo():
    ...
    def primmALG(self):
    	...
    	for value in self.grafo[verIni]:
    		...
    		elementos.append(value[0])
    		elementos.append(value[1])
    		lisOrdnada.append(elementos)
    		elementos=[]
    	print("Visitar: ",lisOrdnada)
    	

    	while len(lisOrdnada)>0:

    		if lisOrdnada[0][1] not in visit:
    			visit.append(lisOrdnada[0][1])
    			for value in self.grafo[lisOrdnada[0][1]]:
    				if value not in lisOrdnada:
    					lisOrdnada.append(value)
    		if verIni not in nvoGrfo:
    			nvoGrfo.update({verIni:[]})
    		if lisOrdnada[0][1] not in nvoGrfo[verIni]:
    			nvoGrfo[verIni].append(lisOrdnada[0])

    		verIni=lisOrdnada[0][1]
    		lisOrdnada.remove(lisOrdnada[0])
    		for i in range(1,len(lisOrdnada)):
	    		j = i
	    		while j > 0 and lisOrdnada[j] < lisOrdnada[j-1]:
	    			lisOrdnada[j], lisOrdnada[j-1] = lisOrdnada[j-1], lisOrdnada[j]
	    			j=j-1
	    	print("Visitados: ",lisOrdnada)
	    	pass
