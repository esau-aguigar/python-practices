...
class Grafo():
    ...
    def primmALG(self):
        ...
        while len(visit)<len(nodos):
            if lisOrdnada[0][0] not in nvoGrfo:
                nvoGrfo.update({lisOrdnada[0][0]:[]})
            if lisOrdnada[0][1][0] not in nvoGrfo:
                nvoGrfo.update({lisOrdnada[0][1][0]:[]})
            if lisOrdnada[0][1][0] not in visit or lisOrdnada[0][0] not in visit:
                nvoGrfo[lisOrdnada[0][0]].append(lisOrdnada[0][1])

            if lisOrdnada[0][0] not in visit:
                visit.append(lisOrdnada[0][0])
                for value in self.grafo[lisOrdnada[0][0]]:
                    elementos.append(lisOrdnada[0][0])
                    elementos.append(value)
                    lisOrdnada.append(elementos)
                    elementos=[]

            if lisOrdnada[0][1][0] not in visit:
                visit.append(lisOrdnada[0][1][0])
                for value in self.grafo[lisOrdnada[0][1][0]]:
                    elementos.append(lisOrdnada[0][1][0])
                    elementos.append(value)
                    lisOrdnada.append(elementos)
                    elementos=[]
